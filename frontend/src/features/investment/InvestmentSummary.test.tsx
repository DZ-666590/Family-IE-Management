...
it('keeps valuation guidance available on demand without a zero-price warning card', async () => {
  ...
  expect(screen.getByText('按当前可用价格计算；缺价持仓不计入市值。')).toBeVisible();
  await user.keyboard('{Escape}');
  expect(screen.getByText('按当前可用价格计算；缺价持仓不计入市值。')).not.toBeVisible();
});
...
it('keeps a meaningful missing-price warning and a working route to quotes', async () => {
  ...
  expect(await screen.findByRole('status')).toHaveTextContent('2 项持仓缺少价格，暂未计入市值。');
