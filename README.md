...
一个部署在服务器上的 Spring Boot + React 家庭财务工作区。它把日常收支、预算、周期账单、资产、A股/港股/美股持仓、汇率与换汇、贷款、提醒和家庭协作放在同一条家庭账轨上，财务数据由服务器计算并持久化到 MySQL 8。

### 多币种投资与账户分类

创建投资可统一搜索选择A股、港股和美股；CNY/HKD/USD按原币账户结算，再用可追溯参考汇率汇总为人民币。支付宝、微信提供独立分类与本地SVG图标。实际换汇一次记录双边本金、费用和到账金额，余额不足整笔失败，修改与冲销保留历史。

使用流程、汇率口径和首次部署保护见 [多币种投资与汇率说明](docs/operations/multicurrency-investments.md)。发布验证成功前，新版不会接受外币开户、换汇或超过两位小数的新单价，避免失败回退后由旧程序误读新数据。
...
- 家庭所有者或管理员可维护 CNY 投资账户，从系统同步的真实 A 股目录中搜索选股，并记录 OPENING/BUY/SELL/DIVIDEND/FEE。用户不再自行注册股票；已有持仓以 OPENING 初始化，不重复扣现金。持仓、成本和收益始终由历史交易推导。
- 行情仅使用收盘日线；工作日上海时间 16:30 刷新，也可手动刷新。有效价格优先选择当天/最近的手工价格，否则使用最后一次已验证的日线快照；响应会显示来源、日期、抓取时间和陈旧状态。
- 配置内部行情适配器后，AKShare 提供沪深京股票目录，BaoStock 提供沪深日线，KLineChart 展示日/周/月线、成交量、MA/MACD 和复权走势。北交所等未覆盖股票明确提示暂无 K 线；未复权价格用于估值，图表复权不修改成交成本。
- 无需家庭用户申请行情 Token。适配器的首次安装与内部地址配置由运维完成，见 [行情适配器运维说明](docs/operations/market-data-adapter.md)。未配置时目录不可用，历史记录与手工价格保留；已有 Tushare 环境配置兼容保留。任何 Token 都不应写入仓库、测试、日志或数据库。
...
账号设置展示服务器统一提供的阿里云百炼 `qwen3.8-max`，所有登录用户共用，个人不再输入 API 密钥。服务器配置与密钥填写步骤见 [服务器 AI 配置教程](docs/operations/system-ai-setup.md)。统一 Java 接口支持文字与受限图片输入，现有 Word/PDF 规则提取功能保留，但尚未接入 Max；详情见 [AI 调用边界](docs/architecture/personal-ai-settings.md)。
