...
public class LoanPrepaymentService {
 ...
 private final LoanAccountingService accounting; private final AccountingRequests requests; private final LoanInstallmentRepository installments;
 LoanPrepaymentService(LoanRepository loans,LoanPrepaymentRepository prepayments,FinancialTransactionRepository transactions,FinancialAccountRepository accounts,CategoryRepository categories,FamilyMemberRepository members,FamilyMutationAuthorization authorization,Clock clock,LoanAccountingService accounting,AccountingRequests requests,LoanInstallmentRepository installments){this.loans=loans;this.prepayments=prepayments;this.transactions=transactions;this.accounts=accounts;this.categories=categories;this.members=members;this.authorization=authorization;this.clock=clock;this.accounting=accounting;this.requests=requests;this.installments=installments;}
 @Transactional public LoanPrepaymentResponse prepay(Authentication authentication,long loanId,LoanPrepaymentRequest request){
  ...
  if(replay!=null){
   ...
   return LoanPrepaymentResponse.from(original,loan);
  }
  ...
  FinancialTransaction transaction=FinancialTransaction.loanPrepayment(access.household(),account(loan,access.context().householdId()),access.membership().getUser(),member(loan,access.context().householdId()),category(loan,access.context().householdId()),amount,paidOn,prepayment.getId(),clock.instant());
  ...
  loan.applyPrincipalPayment(amount,clock.instant());accounting.requireBalance(loan);currentSchedule.forEach(LoanInstallment::cancel); if(loan.getCurrentPrincipalCents()>0)regenerate(loan,paidOn,remainingTerms,nextNo); loans.flush(); requests.record(h,key,digest,prepayment.getId()); return LoanPrepaymentResponse.from(prepayment,loan);
 }
