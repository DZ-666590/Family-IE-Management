...
export function FundingAccountCreator({currency,request,onCreated,onClose}:{currency:string;request:RequestFn;onCreated:(account:Account)=>void;onClose:()=>void}){
 ...
 return <Drawer open title={`新建 ${currency} 资金账户`} draft={{...draft,...kind}} busy={save.isPending} onClose={onClose}>
  <form className="feature-form" onSubmit={e=>{e.preventDefault();if(draft.confirmed&&validPlatform)save.mutate();}}><FormError error={save.error}/>
