...
it('previews the actual recurring cash payment before recording today without changing the due date', async () => {
  ...
  expect(screen.getByRole('heading', { name: '月度计划现金流' })).toBeInTheDocument();
  ...
  expect(screen.getByRole('heading', { name: '执行节奏' })).toBeInTheDocument();
  expect(screen.getByRole('tab', { name: '月度' })).toHaveAttribute('aria-selected', 'true');
  expect(screen.getByRole('tab', { name: '支出' })).toHaveAttribute('aria-selected', 'true');
  await user.click(screen.getByRole('tab', { name: '年度' }));
  expect(screen.getByRole('heading', { name: '年度计划现金流' })).toBeInTheDocument();
  await user.click(screen.getByRole('tab', { name: '月度' }));
  await user.click(screen.getByRole('tab', { name: '收入' }));
  expect(screen.getByRole('heading', { name: '收入分类' })).toBeInTheDocument();
  expect(screen.getByText('暂无执行中的收入分类')).toBeInTheDocument();
