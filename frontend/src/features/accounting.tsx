...
export function PaymentPreview({ account, amount, incoming = false, adjustment = false, compact = false }: { account?: Account; amount: string | null | undefined; incoming?: boolean; adjustment?: boolean;compact?:boolean }) {
  ...
  return <section className="payment-preview" aria-label="账务金额预览">
    ...
    {!compact&&<small>{adjustment ? '更正会冲回原记录后重新入账，余额由服务器按原资金账户核对。' : '仅预览本系统账本余额；记录不会执行银行或券商转账。'}</small>}
  </section>;
}
