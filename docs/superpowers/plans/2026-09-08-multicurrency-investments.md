...
- [x] 实现固定 Frankfurter HTTPS 地址、ECB provider、有限超时、响应体大小限制、重定向禁用、日期/币种/正数/重复字段验证；不发送家庭数据。
- [x] 保存批次及行到 `fx_rate_batches` / `fx_rates`，按最后一个同日版本摘要防重复、修订追加、事务整批写入。抓取在事务外；查询不联网。失败保留旧值并给出状态；最多 4 个自然日标记新鲜。
- [x] GET `/api/exchange-rates?asOf=`、GET `/history?from=&to=` 对家庭成员开放；POST `/refresh` 仅管理角色且 CSRF，服务端限流；用现有 ApiEnvelope 和异常格式。
- [x] 测试实际 H2 持久化、日期选择、历史范围、权限/CSRF、刷新失败、修订与重复批次。更新显式检查最大 Flyway 版本的测试。
- [x] 测试通过后只提交本任务文件。
...
- [x] 测试表格正确展示 `1 USD = 7.000000 CNY`，MISSING 不显示 0，刷新错误不清空旧行情，日期切换不串响应；成员没有刷新按钮。
- [x] 实现汇率页签、30/90天历史、日期查询和状态反馈，不增加侧边栏入口。
- [x] 测试支付宝/微信选项保存仍为 WALLET+provider；旧未细分钱包保持原值，键盘选择可用。
- [x] 实现共享图标身份、分类卡片和付款列表，复用现有主题；品牌 SVG 仅使用许可可核对资源，否则文字徽标。
- [x] 前端测试、类型检查及构建通过，只提交拥有的文件。
...
- [x] 测试 `1 USD 借 / 1 CNY 贷` 拒绝；同币借贷相等通过；旧 CNY 重放不新增凭证；外币科目不可改币种。
- [x] 迁移补 CNY、按币种校验平衡，公共损益/权益科目拆分；外币功能开关默认关闭。
- [x] 现金余额、日序校验、账户初始化保留原币；仅在同币账户间进行普通互转；钱包平台 CNY 校验服务端执行。
- [x] 贷款/资产/周期规则入口明确拒绝非 CNY；投资账户必须与资金账户、证券币种一致。
- [x] 回归账本、贷款、资产、账户、幂等与迁移测试后提交。
...
- [x] 测试 CNY本金7000、手续费10、USD到账1000：余额分别-7010/+1000，费用10，换汇本金不算支出。
- [x] 一个事务内按币种各自平衡并关联同一来源；锁家庭/账户，余额不足全部回滚。
- [x] 实现双边预览、实际汇率与参考汇率分开；相同请求重放成功，不同载荷拒绝；冲销遇到账款已花掉时整笔拒绝。
- [x] 测试权限、跨家庭、日期、金额上界、并发重放、更正与冲销，再提交。
...
- [x] 测试同代码跨市场不冲突，HKD证券不能从USD账户扣款；期初不扣现金，卖出不超过持仓。
- [x] 支持六位单价、四位数量，金额最终两位；记录不可变参考汇率/历史人民币成本，最后清仓结清尾差。
- [x] 创建投资统一 A/HK/US 搜索，切市场清选项，无同币账户时在流程内引导建立并返回草稿。
- [x] 测试缺行情可按实际价记账、缺汇率报告标不完整；当前汇率变动不改历史成本；买卖/分红/费用与更正/冲销回归后提交。
...
- [x] 测试 CNY100+USD100*7=800，而非200；缺USD汇率时总额不伪报100；历史报告不使用未来数据。
- [x] 补齐所有 SUM/金额格式化/快照/CSV消费者，人民币总收益使用历史成本而非今天汇率乘原币收益；保留旧快照版本。
- [x] 验证完整H2/MySQL迁移、纯CNY回归、前端测试/typecheck/build、后端针对性全套；记录证据。
- [ ] 完整验证后才打开外币功能开关。发布须经过授权、备份和云端FX连通验证；CI 与线上版本分别核对，不回滚旧程序写入多币种数据。

## 当前进度

- [x] Task 1：参考汇率、历史批次、自动初始化/定时刷新、缺失引用补齐、权限和限流。
- [x] Task 2：品牌SVG、账户分类卡片、账户列表及初始化/付款预览统一身份、汇率表。
- [x] Task 3：原币账本、逐币种平衡、同币种账户约束、人民币贷款/资产/周期规则隔离。
- [x] Task 4：实际换汇前后端、双边预览、幂等和不确定结果重试、更正/冲销、并发余额保护。
- [x] Task 5：统一A/HK/US选股、自动证券登记、六位单价、持仓行情、历史人民币成本与收益。
- [x] Task 6 实现和自动化：汇总/预算/历史/导出、缺失汇率的明确状态、MySQL隔离迁移验证和发布保护。
- [ ] 正式发布与线上视觉验收：等待推送部署授权，不代表仍有功能开发缺项。

账户选项保留原生控件以维持现有键盘与还款流程；分类卡片、账户列表、初始化与选中付款预览统一显示品牌图标，不在原生option中放置无效SVG。

### 完整实现交付说明

功能提交为4ab5466，已在本地整合远端PR8并保留合同提取功能。合并版本后端666项测试（2跳过）无失败，前端278项、类型检查及构建通过。最终实现新增V29–V37；MySQL8结构及合成数据验证通过，临时库已删除。业务库没有改变。服务器可访问参考汇率源，含非发布日自动取最近有效日期的响应已核对。

首次发布使用部署接收器成功标记与包内commit匹配保护：只有验证成功才开启外币写入和超过两位小数的新单价。正常CI发布会自动解锁；不需要手动绕过。测试环境显式关闭此部署匹配限制且关闭网络定时任务。

下面保留分批过程中的历史记录，不能将早期“未完成”状态误读为最终交付状态。
