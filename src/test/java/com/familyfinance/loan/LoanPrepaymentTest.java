...
class LoanPrepaymentTest {
 ...
 private long confirm(MockHttpSession s,long id)throws Exception{return body(mvc.perform(post("/api/loan-installments/{id}/confirm",id).session(s).with(csrf()).contentType(MediaType.APPLICATION_JSON).content("{\"paidOn\":\"2026-09-03\"}")).andExpect(status().isOk()).andReturn()).path("data").path("confirmedTransactionId").asLong();}
 ...
 private long create(MockHttpSession s)throws Exception {long a=jdbc.queryForObject("select min(id) from financial_accounts where household_id=1",Long.class),c=jdbc.queryForObject("select min(id) from categories where household_id=1 and kind='EXPENSE'",Long.class),m=jdbc.queryForObject("select min(id) from family_members where household_id=1",Long.class),u=jdbc.queryForObject("select id from app_users where email='demo@local.family'",Long.class);String b="{\"fundingMode\":\"OPENING\",\"accountingOn\":\"2026-01-01\",\"name\":\"提前还款\",\"type\":\"OTHER\",\"memberId\":"+m+",\"assignedUserId\":"+u+",\"paymentAccountId\":"+a+",\"paymentCategoryId\":"+c+",\"principal\":\"1000.00\",\"annualRate\":0.120000,\"termMonths\":3,\"repaymentMethod\":\"EQUAL_PAYMENT\",\"startOn\":\"2026-08-01\"}";return body(mvc.perform(post("/api/loans").session(s).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(b)).andExpect(status().isCreated()).andReturn()).path("data").path("id").asLong();}
