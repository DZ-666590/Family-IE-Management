...
export interface PortfolioPosition { valuationStatus?: 'QUOTED' | 'COST_ESTIMATE' | 'CLOSED'; estimatedValue?: string; accountId: number; accountName: string; brokerName: string; securityId: number; tsCode: string; name: string; quantity: number; averageCost: string; cost: string; price: string | null; marketValue: string | null; realizedProfit: string; unrealizedProfit: string | null; totalProfit: string | null; allocationPercent: string | null; source: 'TUSHARE' | 'MANUAL' | null; tradeDate: string | null; fetchedAt: string | null; stale: boolean; error: string | null }
export interface Portfolio { positions: PortfolioPosition[]; totals: { cost: string; estimatedValue?: string; marketValue: string | null; realizedProfit: string; unrealizedProfit: string | null; totalProfit: string | null; unpricedPositions: number } }

export type PrepaymentStrategy = 'REDUCE_TERM' | 'REDUCE_PAYMENT';
export interface LoanPrepaymentSchedule { principalAmount: string; periodCount: number; maturityOn: string; nextPaymentOn: string; nextPaymentAmount: string; totalInterest: string; repaymentTotal: string; schedule: Array<{ installmentNo: number; dueOn: string; principal: string; interest: string; paymentAmount: string; remainingPrincipal: string }> }
...
export interface LoanPrepayment { strategy?: PrepaymentStrategy | null; id: number; transactionId: number; amount: string; remainingPrincipal: string; status: Loan['status']; paidOn: string; principalAmount: string; interestAmount: string; cashAmount: string; operationKind: 'PREPAYMENT' | 'PAYOFF'; paymentAccountId: number; scheduledRepaymentTotal: string; remainingRepaymentTotal: string; paidRepaymentTotal: string }
export interface LoanInstallment { cancelledByPrepaymentId?: number | null; paymentAccountId?: number | null; paidOn?: string | null; cashAmount?: string; id: number; installmentNo: number; dueOn: string; principal: string; interest: string; status: 'PENDING' | 'PAID' | 'CANCELLED'; confirmedTransactionId: number | null }
...
export interface NetWorth { asOf?: string; accountingBasis?: string; cumulativeAssetValuationChange?: string; asset: string; liability: string; netWorth: string; allocation: Array<{ type: string; amount: string; sharePercent: string }>; debtRatioPercent: string; budget: { activeBudgetCount: number; planned: string; spent: string; nearLimitCount: number; overLimitCount: number }; investment: { marketValue: string | null; estimatedValue?: string; positionCount: number; unpricedPositionCount: number; manualPrice: boolean; stalePrice: boolean; missingPrice: boolean }; history: NetWorthHistory[] }
export interface NetWorthHistory { snapshotOn: string; asset: string; liability: string; netWorth: string; accountingBasis: string; valuationEstimated: boolean; unpricedPositions: number }
