...
class LoanRepaymentApiTest {
 ...
 private long create(MockHttpSession s,String start)throws Exception { long household=jdbc.queryForObject("select household_id from app_users where email='demo@local.family'",Long.class);long account=jdbc.queryForObject("select min(id) from financial_accounts where household_id=?",Long.class,household), category=jdbc.queryForObject("select min(id) from categories where household_id=? and kind='EXPENSE'",Long.class,household), member=jdbc.queryForObject("select min(id) from family_members where household_id=?",Long.class,household), user=jdbc.queryForObject("select id from app_users where email='demo@local.family'",Long.class); String b="{\"fundingMode\":\"OPENING\",\"accountingOn\":\"2026-01-01\",\"name\":\"测试贷款\",\"type\":\"OTHER\",\"memberId\":"+member+",\"assignedUserId\":"+user+",\"paymentAccountId\":"+account+",\"paymentCategoryId\":"+category+",\"principal\":\"1000.00\",\"annualRate\":0.120000,\"termMonths\":3,\"repaymentMethod\":\"EQUAL_PAYMENT\",\"startOn\":\""+start+"\"}";return body(mvc.perform(post("/api/loans").session(s).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(b)).andExpect(status().isCreated()).andReturn()).path("data").path("id").asLong(); }
