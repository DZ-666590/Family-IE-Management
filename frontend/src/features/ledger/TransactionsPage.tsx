...
export function TransactionsPage({ request, role, userId }: { request: RequestFn; role: HouseholdRole; userId: number }) {
  ...
  return <PageScaffold title="收支明细" primaryAction={action}>
    ...
    <Drawer draft={draft} sessionKey={draft?.id} busy={save.isPending} onSessionStart={save.reset} open={Boolean(draft)} title={draft?.id ? '编辑收支' : '记一笔'} description="保存后将同步更新相关预算和家庭总览。" onClose={() => setDraft(null)}>{draft && <TransactionForm draft={draft} accounts={accountOptions.data ?? []} categories={flatCategories} members={members.data ?? []} error={save.error} saving={save.isPending} onChange={setDraft} onSubmit={() => save.mutate(draft)} />}</Drawer>
    <Drawer draft={accountDraft} sessionKey={accountDraft?.id} busy={saveAccount.isPending} onSessionStart={saveAccount.reset} open={Boolean(accountDraft)} title={accountDraft?.id ? '编辑账户' : '新建账户'} onClose={() => setAccountDraft(null)}>{accountDraft && <form className="feature-form" onSubmit={e => { e.preventDefault(); saveAccount.mutate(accountDraft); }}><FormError error={saveAccount.error} /><label>账户名称<input name="name" required value={accountDraft.name} onChange={e => setAccountDraft({ ...accountDraft, name: e.target.value })} /></label><label>账户类型<select name="type" value={accountDraft.type} onChange={e => setAccountDraft({ ...accountDraft, type: e.target.value as AccountType })}><option value="CASH">现金</option><option value="BANK">银行卡</option><option value="WALLET">电子钱包</option></select></label><label>币种<input name="currency" value={accountDraft.currency} onChange={e => setAccountDraft({ ...accountDraft, currency: e.target.value })} /></label><label>期初余额<input name="openingBalance" inputMode="decimal" value={accountDraft.openingBalance} onChange={e => setAccountDraft({ ...accountDraft, openingBalance: e.target.value })} /></label><Button htmlType="submit" theme="solid" type="primary" loading={saveAccount.isPending}>保存账户</Button></form>}</Drawer>
    ...
    <ConfirmDialog open={deleteId !== null} loading={remove.isPending} title="删除这笔收支？" detail={<><p>该操作会重新计算相关预算和总览，且无法撤销。</p><FormError error={remove.error} /></>} danger confirmLabel="删除收支" onClose={() => { setDeleteId(null); remove.reset(); }} onConfirm={() => deleteId !== null && remove.mutate(deleteId)} />
  </PageScaffold>;
}
...
function TransactionForm({ draft, accounts, categories, members, error, saving, onChange, onSubmit }: { draft: TransactionDraft; accounts: Account[]; categories: Category[]; members: Member[]; error: unknown; saving: boolean; onChange: (draft: TransactionDraft) => void; onSubmit: () => void }) {
  ...
  return <form className="feature-form" onSubmit={submit}><FormError error={error} />
    <div className="choice-row" role="radiogroup" aria-label="收支类型"><label><input name="kind" type="radio" checked={draft.kind === 'expense'} onChange={() => onChange({ ...draft, kind: 'expense', categoryId: '' })} />支出</label><label><input name="kind" type="radio" checked={draft.kind === 'income'} onChange={() => onChange({ ...draft, kind: 'income', categoryId: '' })} />收入</label></div>
    ...
    <label>日期<input name="occurredOn" aria-label="日期" required type="date" value={draft.occurredOn} onChange={e => onChange({ ...draft, occurredOn: e.target.value })} /></label>
    <label>账户<select name="accountId" aria-label="账户" required value={draft.accountId} onChange={e => onChange({ ...draft, accountId: e.target.value })}><option value="">请选择账户</option>{accounts.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
