...
export function TransactionsPage({ request, role }: { request: RequestFn; role: HouseholdRole; userId: number }) {
  ...
  return <PageScaffold title="收支明细" primaryAction={action}>
    ...
    <Drawer open={Boolean(draft)} title={draft?.id ? '编辑收支' : '记一笔'} description="保存后将同步更新相关预算和家庭总览。" onClose={() => setDraft(null)}>{draft && <TransactionForm draft={draft} accounts={accountOptions.data ?? []} categories={flatCategories} members={members.data ?? []} error={save.error} saving={save.isPending} onChange={setDraft} onSubmit={() => save.mutate(draft)} />}</Drawer>
    ...
    <Drawer open={Boolean(categoryDraft)} title={categoryDraft?.id ? '编辑分类' : '新建分类'} onClose={() => setCategoryDraft(null)}>{categoryDraft && <form className="feature-form" onSubmit={e => { e.preventDefault(); saveCategory.mutate(categoryDraft); }}><FormError error={saveCategory.error} /><label>分类名称<input required value={categoryDraft.name} onChange={e => setCategoryDraft({ ...categoryDraft, name: e.target.value })} /></label><label>收支类型<select value={categoryDraft.kind} onChange={e => setCategoryDraft({ ...categoryDraft, kind: e.target.value as TransactionKind })}><option value="expense">支出</option><option value="income">收入</option></select></label><label>上级分类<select value={categoryDraft.parentId} onChange={e => setCategoryDraft({ ...categoryDraft, parentId: e.target.value })}><option value="">一级分类</option>{parentCategories.filter(item => item.kind === categoryDraft.kind && item.id !== categoryDraft.id).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>标记颜色<input type="color" value={categoryDraft.color} onChange={e => setCategoryDraft({ ...categoryDraft, color: e.target.value })} /></label><Button htmlType="submit" theme="solid" type="primary" loading={saveCategory.isPending}>保存分类</Button></form>}</Drawer>
