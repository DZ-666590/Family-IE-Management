...
export function AccountTypePicker({value,onChange}:{value:AccountClassification;onChange:(value:AccountClassification)=>void}){
  const id=useId();
  ...
  return <fieldset className="account-type-picker"><legend>账户分类</legend><div>{choices.map(option=><label key={option.label}>
