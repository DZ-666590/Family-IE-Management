...
export function RecurringPage({ request, role, userId }: { request: RequestFn; role: HouseholdRole; userId: number }) {
  ...
  return <PageScaffold title="周期账单" description="把固定收入与支出变成可确认的计划，到期后由你决定何时入账。" primaryAction={manager ? { label: '新建周期规则', onClick: () => setDraft(newRule()) } : undefined}>
    <RecurringOverview pendingCount={occurrences.data?.totalElements} overdueCount={occurrences.data ? overdueCount : undefined} selectableCount={selectableItems.length} activeRules={ruleOptions.data ?? []} loading={occurrences.isLoading || ruleOptions.isLoading} />
...
function RecurringOverview({ pendingCount, overdueCount, selectableCount, activeRules, loading }: {
  pendingCount?: number;
  overdueCount?: number;
