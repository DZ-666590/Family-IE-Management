...
class LoanPrepaymentStrategyApiTest {
 ...
 @Test void reducePaymentUsesLiteralInterestAndLegacyDefaultWithoutAdditionalDebit()throws Exception{
  long loan=create("0.12");var q=data(quote(loan,"300.00","REDUCE_PAYMENT",account).andExpect(status().isOk()).andExpect(jsonPath("$.data.after.totalInterest").value("59.56")).andExpect(jsonPath("$.data.after.periodCount").value(12)).andExpect(jsonPath("$.data.after.schedule[11].paymentAmount").value("80.00")).andExpect(jsonPath("$.data.after.maturityOn").value("2026-12-31")).andReturn());
