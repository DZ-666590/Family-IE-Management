...
export function TransactionsPage({ request, role }: { request: RequestFn; role: HouseholdRole; userId: number }) {
  ...
  return <PageScaffold title="收支明细" primaryAction={action}>
    ...
    <Drawer open={Boolean(draft)} title={draft?.id ? '编辑收支' : '记一笔'} description="保存后将同步更新相关预算和家庭总览。" onClose={() => setDraft(null)}>{draft && <TransactionForm draft={draft} accounts={accounts.data?.items ?? []} categories={flatCategories} members={members.data ?? []} error={save.error} saving={save.isPending} onChange={setDraft} onSubmit={() => save.mutate(draft)} />}</Drawer>
