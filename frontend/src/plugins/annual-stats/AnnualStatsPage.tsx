...
export default function AnnualStatsPage({ request }: { request: RequestFn }) {
  ...
  return <PageScaffold title="年度统计">
    ...
    <QueryState loading={report.isLoading} error={report.error}>
      ...
      <DataPanel title="月度现金流" meta="全年 12 个月 · 零记录月份不绘制柱形"><FlowChart label="年度月度收入与支出" points={report.data?.months.map(row=>({label:row.month+'月',income:row.income,expense:row.expense})) ?? []}/></DataPanel>
