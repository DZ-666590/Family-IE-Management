...
- 已确认回滚的数据库死锁，在完整业务事务之外最多重试三次，每次重新授权和读取；内核不能只重试分录。余额不足、校验失败、网络故障或提交结果不明不得自动重试。
...
- 零金额期初是确认事件，不伪造零金额收支分录；确认元数据应保留，真实余额仍为零。
- 资金账户 `openingOn` 表示该日开始记账前的余额。该日前的活动已包含在期初中，不能再次录入正式现金业务；同日业务允许。修改开账日不得移到已有有效现金业务之后。冲销/替换旧现金业务仍须校验原账户可操作，不能向已归档账户恢复余额。
